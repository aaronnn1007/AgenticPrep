...
class QuestionModel(BaseModel):
    ...
    topic: str = Field(...,
                       description="Topic category (e.g., 'algorithms', 'system design')")
    difficulty: float = Field(..., ge=0.0, le=1.0,
                              description="Difficulty rating from 0 to 1")
    intent: List[str] = Field(default_factory=list,
                              description="Expected answer components")
...
class VoiceAnalysisModel(BaseModel):
    ...
    filler_ratio: float = Field(
        0.0, ge=0.0, le=1.0, description="Ratio of filler words to total words")
    tone: str = Field(
        "neutral", description="Detected tone (confident, neutral, nervous)")
    is_computed: bool = Field(
        False, description="True only when metrics were produced by real analysis, not defaults or fallback")


class AnswerQualityModel(BaseModel):
    """Quality assessment of the answer content."""
    relevance: float = Field(
        0.0, ge=0.0, le=1.0, description="How relevant the answer is to the question")
    correctness: float = Field(
        0.0, ge=0.0, le=1.0, description="Technical correctness of the answer")
    depth: float = Field(0.0, ge=0.0, le=1.0,
                         description="Depth of explanation")
    structure: float = Field(
        0.0, ge=0.0, le=1.0, description="Logical structure and organization")
    gaps: List[str] = Field(default_factory=list,
                            description="Missing concepts or information")


class BodyLanguageModel(BaseModel):
    """Body language and visual behavior analysis."""
    eye_contact: float = Field(
        0.0, ge=0.0, le=1.0, description="Consistency of eye contact")
    posture_stability: float = Field(
        0.0, ge=0.0, le=1.0, description="Stability and confidence in posture")
    facial_expressiveness: float = Field(
        0.0, ge=0.0, le=1.0, description="Appropriate facial expressions")
    distractions: List[str] = Field(
        default_factory=list, description="Observed distracting behaviors")


class ConfidenceBehaviorModel(BaseModel):
    """Confidence and behavioral inference."""
    confidence: float = Field(
        0.0, ge=0.0, le=1.0, description="Overall confidence level")
    nervousness: float = Field(
        0.0, ge=0.0, le=1.0, description="Signs of nervousness")
    professionalism: float = Field(
        0.0, ge=0.0, le=1.0, description="Professional demeanor")
    behavioral_flags: List[str] = Field(
        default_factory=list, description="Notable behavioral patterns")


class ScoresModel(BaseModel):
    """Final aggregated scores."""
    technical: float = Field(0.0, ge=0.0, le=100.0,
                             description="Technical competency score (0-100)")
    communication: float = Field(
        0.0, ge=0.0, le=100.0, description="Communication effectiveness score (0-100)")
    behavioral: float = Field(
        0.0, ge=0.0, le=100.0, description="Behavioral professionalism score (0-100)")
...
class InterviewState(BaseModel):
    ...
    recommendations: RecommendationsModel = Field(
        default_factory=RecommendationsModel)

    # Internal metadata
    audio_path: Optional[str] = Field(
        None, description="Path to uploaded audio file")
    video_path: Optional[str] = Field(
        None, description="Path to uploaded video file")
